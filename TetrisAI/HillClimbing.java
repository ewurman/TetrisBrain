...
public class HillClimbing {
	int exploration_constant = 3;
	// better  as array of weights
	...
	public void SoftStochasticSearch(int maxHeight, int heightRange, int holes, int roughness, int blockades){
		...
		for (int i; i<n; i++){
			int current_score = ;//score of game with current

			double increment = getRandomNumberInRange();
			int sample = getRandomNumberInRange(0, 4);

			int[] weights = [maxHeight, heightRange, holes, roughness, blockades];
			weights[sample] += increment;
			...
			double average = ;// average of first, second, third
